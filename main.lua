...
local function finishLoading()
	...
	if not vape.Load then
		warn('[euphoria] vape.Load is nil skipping load')
		return
	end
	vape:Load()
...
if not guiFunc then
	...
	error('[euphoria] syntax error in ' .. gui .. '.lua' .. '\n' .. errMsg .. context)
end
vape = guiFunc()
if not vape then
	error('[euphoria] GUI returned nil file may be corrupted try deleting euphoria/guis/' .. gui .. '.lua and reinjecting.')
end
if not vape.Load then
	if delfile then pcall(function() delfile('euphoria/guis/' .. gui .. '.lua') end) end
	error('[euphoria] gui file corrupted (missing load) reinject..')
end
if not vape.Init and not vape.Load then
	error('[euphoria] failed to initialize properly reinject to fix this bs')
end
shared.vape = vape
task.wait(0.1)
...
if not shared.VapeIndependent then
	...
	if isfile('euphoria/games/' .. gameFileId .. '.lua') then
		...
		if not gameFunc then
			...
			error('[euphoria] syntax error in ' .. gameFileId .. '.lua\n' .. msg .. ctx)
		end
		gameFunc(...)
	else
