local loadstring = function(...)
	...
	if err and vape then
		vape:CreateNotification('richspec', 'Failed to load : '..err, 30, 'alert')
	end
	return res
end
	...
	ChatSpammer = vape.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				local ind = 1
				repeat
					local message = (#Lines.ListEnabled > 0 and Lines.ListEnabled[math.random(1, #Lines.ListEnabled)] or 'richspec on top')
...
run(function()
	...
	local originalZoom = lplr.CameraMaxZoomDistance
	InfiniteZoom = vape.Categories.Utility:CreateModule({
		Name = 'InfiniteZoom',
		Function = function(callback)
			if callback then
				lplr.CameraMaxZoomDistance = ZoomSlider.Value
			else
				lplr.CameraMaxZoomDistance = originalZoom
			end
		end,
		Tooltip = 'Allows you to zoom out further than normal'
	})

	ZoomSlider = InfiniteZoom:CreateSlider({
		Name = 'Max Distance',
		Min = 128,
		Max = 2000,
		Default = 500,
